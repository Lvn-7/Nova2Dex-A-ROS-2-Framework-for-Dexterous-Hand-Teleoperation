...
public:
  ...
  {
    // The Inspire protocol uses -1 (0xFFFF) to mean "keep unchanged".
    // Preserve that sentinel instead of clamping it to the closed position.
    Eigen::Matrix<int16_t, 6, 1> q_int16;
    for (int i = 0; i < 6; ++i)
    {
      q_int16(i) = q(i) < 0.0
          ? static_cast<int16_t>(-1)
          : static_cast<int16_t>(std::max(0.0, std::min(1.0, q(i))) * 1000.0);
    }
  ...
  int16_t GetPosition(Eigen::Matrix<double, 6, 1> & q)
  {
    ...
    usleep(5000);
    const ssize_t len = serial_->recv(recvBuff, 20);

    if(len != 20)
    {
      if (++position_read_failures_ % 100 == 1)
        std::cerr << "Inspire hand ID " << static_cast<int>(id)
                  << " position response bytes=" << len << " (expected 20)" << std::endl;
      return 1;
    }
...
private:
  ...
  std::size_t position_read_failures_{0};
