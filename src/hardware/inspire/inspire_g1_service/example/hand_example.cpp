...
public:
        ...
        const Eigen::Matrix<float, 6, 1>& left_angles)
    {
        ...
        {
            right_cmd.cmds()[i].q() = right_angles(i);
            left_cmd.cmds()[i].q() = left_angles(i);
        }
        right_publisher->Write(right_cmd);
        left_publisher->Write(left_cmd);
    }
...
private:
    void InitDDS_()
    {
        right_publisher = std::make_shared<unitree::robot::ChannelPublisher<unitree_go::msg::dds_::MotorCmds_>>(
            "rt/inspire/right/cmd");
        right_publisher->InitChannel();
        right_cmd.cmds().resize(6);
        left_publisher = std::make_shared<unitree::robot::ChannelPublisher<unitree_go::msg::dds_::MotorCmds_>>(
            "rt/inspire/left/cmd");
        left_publisher->InitChannel();
        left_cmd.cmds().resize(6);
    ...
    // DDS parameters
    std::mutex mtx;
    unitree::robot::ChannelPublisherPtr<unitree_go::msg::dds_::MotorCmds_> right_publisher;
    unitree::robot::ChannelPublisherPtr<unitree_go::msg::dds_::MotorCmds_> left_publisher;
    unitree_go::msg::dds_::MotorCmds_ right_cmd;
    unitree_go::msg::dds_::MotorCmds_ left_cmd;
