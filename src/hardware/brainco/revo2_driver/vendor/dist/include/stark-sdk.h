...
/// Maximum pressure points per Revo3 touch module (ThumbPad has 44)
constexpr static const uintptr_t REVO3_TOUCH_MAX_POINTS = 46;

/// Maximum measurement points per Force3D finger (up to 31)
constexpr static const uintptr_t FORCE3D_MAX_POINTS = 31;

/// ArrayPressure register count (5 fingers × 5 params = 25)
constexpr static const uintptr_t ARRAY_PRESSURE_DATA_COUNT = 25;
...
enum HandType : int32_t {
  HAND_TYPE_HANDTYPE_UNUSED = 0,
  HAND_TYPE_REGULAR_RIGHT = 1,
  HAND_TYPE_REGULAR_LEFT = 2,
  HAND_TYPE_MINI_RIGHT = 3,
  HAND_TYPE_MINI_LEFT = 4,
};
...
/// Opaque handle for Force3DTouchDataBuffer
struct CForce3DTouchDataBuffer;
...
/// Opaque handle for Revo3MotorStatusBuffer (Revo3: 23 motors, float values)
struct CRevo3MotorStatusBuffer;

/// Opaque handle for Revo3TouchDataBuffer (Revo3: 11 modules tactile data)
...
/// Revo3 motor status data for all 21 motors (C API)
/// Call `free_revo3_motor_status_data` to release.
struct CRevo3MotorStatusData {
  /// Motor status codes (21 values)
  uint16_t statuses[21];
  /// Motor positions in float (21 values, degrees)
  float positions[21];
  /// Motor velocities in float (21 values)
  float velocities[21];
  /// Motor currents in float (21 values, Amperes)
  float currents[21];
  /// Motor error codes (21 values)
  uint16_t errors[21];
};

/// Revo3 touch data for C API (fixed-size arrays for all 11 tactile sensor modules)
///
/// summary: 16 u16 force values (palm + 5 fingers × 3 pads)
/// modules: 11 tactile sensor arrays, each containing per-point pressure data:
///   [0]  Palm        — 29 pts (addr 20200–20228)
///   [1]  ThumbTip    — 22 pts (addr 20250–20271)
///   [2]  ThumbPad    — 44 pts (addr 20300–20343)
///   [3]  IndexTip    — 21 pts (addr 20350–20370)
///   [4]  IndexPad    — 29 pts (addr 20400–20428)
///   [5]  MiddleTip   — 21 pts (addr 20450–20470)
///   [6]  MiddlePad   — 29 pts (addr 20500–20528)
///   [7]  RingTip     — 21 pts (addr 20550–20570)
///   [8]  RingPad     — 29 pts (addr 20600–20628)
///   [9]  PinkyTip    — 21 pts (addr 20650–20670)
///   [10] PinkyPad    — 29 pts (addr 20700–20728)
/// module_counts: actual pressure point count per module
struct CRevo3TouchData {
  uint16_t summary[16];
  /// 11 tactile sensor arrays, each with up to REVO3_TOUCH_MAX_POINTS pressure points
  uint16_t modules[11][REVO3_TOUCH_MAX_POINTS];
  /// Actual pressure point count per module
...
/// C-compatible Force3D finger data
struct CForce3DFingerData {
  /// Resultant force (summary)
  CForce3DPoint summary;
  /// Distributed measurement points
  CForce3DPoint points[FORCE3D_MAX_POINTS];
  /// Actual number of valid points
  uint16_t point_count;
};

/// C-compatible Force3D touch data (4 fingers: index, middle, ring, pinky)
struct CForce3DTouchData {
  CForce3DFingerData index;
  CForce3DFingerData middle;
  CForce3DFingerData ring;
  CForce3DFingerData pinky;
};

/// C-compatible ArrayPressure touch data
struct CArrayPressureTouchItem {
  /// Sensor status bits (5 bits, one per finger, 1=abnormal)
  uint16_t sensor_status;
  /// Warmup complete flag (1=complete, 0=warming up)
  uint8_t warmup_complete;
  /// Raw register data (25 values: 5 fingers × [Fx,Fy,Fz,Mx,My])
  uint16_t data[ARRAY_PRESSURE_DATA_COUNT];
  /// Actual number of valid data values
  uint16_t data_count;
};
...
struct CDeviceInfo {
  ...
  /// Hardware version string (null-terminated, Revo3 only)
...
/// Create a new Revo3 motor status buffer
///
/// @param max_size Maximum buffer capacity
/// @return Pointer to CRevo3MotorStatusBuffer, or NULL on failure
CRevo3MotorStatusBuffer *revo3_motor_buffer_new(uintptr_t max_size);

/// Free a Revo3 motor status buffer
void revo3_motor_buffer_free(CRevo3MotorStatusBuffer *buffer);

/// Pop all Revo3 motor status data from buffer
///
/// @param buffer Pointer to CRevo3MotorStatusBuffer
/// @param out_data Output array of CRevo3MotorStatusData
/// @param max_count Maximum number of items to read
/// @return Number of items read, or -1 on failure
int revo3_motor_buffer_pop_all(CRevo3MotorStatusBuffer *buffer,
                               CRevo3MotorStatusData *out_data,
                               uintptr_t max_count);

/// Get number of items in Revo3 motor buffer
uintptr_t revo3_motor_buffer_len(const CRevo3MotorStatusBuffer *buffer);

/// Clear Revo3 motor buffer
void revo3_motor_buffer_clear(CRevo3MotorStatusBuffer *buffer);

/// Create a new data collector - Revo3 Basic version (Revo3 motor only)
///
/// @param handle Device handler
/// @param motor_buffer Revo3 motor status buffer
/// @param slave_id Slave ID (default 1)
/// @param motor_frequency Motor sampling frequency in Hz (default 1000)
/// @param enable_stats Whether to print statistics (1=true, 0=false)
/// @return Pointer to CDataCollector, or NULL on failure
...
/// Create a new Revo3 touch data buffer
///
/// @param max_size Maximum buffer capacity
/// @return Pointer to CRevo3TouchDataBuffer, or NULL on failure
CRevo3TouchDataBuffer *revo3_touch_buffer_new(uintptr_t max_size);

/// Free a Revo3 touch data buffer
void revo3_touch_buffer_free(CRevo3TouchDataBuffer *buffer);

/// Pop all Revo3 touch data from buffer
///
/// @param buffer Pointer to CRevo3TouchDataBuffer
/// @param out_data Output array of CRevo3TouchData
/// @param max_count Maximum number of items to read
/// @return Number of items read, or -1 on failure
...
/// Get number of items in Revo3 touch buffer
uintptr_t revo3_touch_buffer_len(const CRevo3TouchDataBuffer *buffer);

/// Clear Revo3 touch buffer
void revo3_touch_buffer_clear(CRevo3TouchDataBuffer *buffer);

/// Create a new data collector - Revo3 Full version (Revo3 motor + touch)
///
/// @param handle Device handler
/// @param motor_buffer Revo3 motor status buffer
/// @param touch_buffer Revo3 touch data buffer
/// @param slave_id Slave ID (default 1)
/// @param motor_frequency Motor sampling frequency in Hz (default 60)
/// @param touch_frequency Touch sampling frequency in Hz (default 5)
/// @param enable_stats Whether to print statistics (1=true, 0=false)
/// @return Pointer to CDataCollector, or NULL on failure
...
/// Create a new Force3D touch data buffer
CForce3DTouchDataBuffer *force3d_touch_buffer_new(uintptr_t max_size);

/// Free a Force3D touch data buffer
void force3d_touch_buffer_free(CForce3DTouchDataBuffer *buffer);

/// Pop all Force3D touch data from buffer
///
/// @param buffer Pointer to CForce3DTouchDataBuffer
/// @param out_data Output array of CForce3DTouchData
/// @param max_count Maximum number of items to read
/// @return Number of items read, or -1 on failure
int force3d_touch_buffer_pop_all(CForce3DTouchDataBuffer *buffer,
                                 CForce3DTouchData *out_data,
                                 uintptr_t max_count);

/// Get number of items in Force3D touch buffer
uintptr_t force3d_touch_buffer_len(const CForce3DTouchDataBuffer *buffer);

/// Clear Force3D touch buffer
void force3d_touch_buffer_clear(CForce3DTouchDataBuffer *buffer);

/// Create a new data collector - Force3D touch version
///
/// @param handle Device handler
/// @param motor_buffer Motor status buffer
/// @param touch_buffer Force3D touch data buffer
/// @param slave_id Slave ID (default 1)
/// @param motor_frequency Motor sampling frequency in Hz (default 1000)
/// @param touch_frequency Touch sampling frequency in Hz (default 10)
/// @param enable_stats Whether to print statistics (1=true, 0=false)
/// @return Pointer to CDataCollector, or NULL on failure
CDataCollector *data_collector_new_force3d(DeviceHandler *handle,
                                           CMotorStatusBuffer *motor_buffer,
                                           CForce3DTouchDataBuffer *touch_buffer,
                                           unsigned char slave_id,
                                           uint32_t motor_frequency,
                                           uint32_t touch_frequency,
                                           int enable_stats);

/// Create a new ArrayPressure touch data buffer
CArrayPressureTouchItemBuffer *array_pressure_touch_buffer_new(uintptr_t max_size);

/// Free an ArrayPressure touch data buffer
void array_pressure_touch_buffer_free(CArrayPressureTouchItemBuffer *buffer);

/// Pop all ArrayPressure touch data from buffer
///
/// @param buffer Pointer to CArrayPressureTouchItemBuffer
/// @param out_data Output array of CArrayPressureTouchItem
/// @param max_count Maximum number of items to read
/// @return Number of items read, or -1 on failure
int array_pressure_touch_buffer_pop_all(CArrayPressureTouchItemBuffer *buffer,
                                        CArrayPressureTouchItem *out_data,
                                        uintptr_t max_count);

/// Get number of items in ArrayPressure touch buffer
uintptr_t array_pressure_touch_buffer_len(const CArrayPressureTouchItemBuffer *buffer);

/// Clear ArrayPressure touch buffer
...
/// @param handle Device handler
/// @param motor_buffer Motor status buffer
/// @param touch_buffer ArrayPressure touch data buffer
/// @param slave_id Slave ID (default 1)
/// @param motor_frequency Motor sampling frequency in Hz (default 1000)
/// @param touch_frequency Touch sampling frequency in Hz (default 10)
/// @param enable_stats Whether to print statistics (1=true, 0=false)
...
void stark_revo3_set_touch_module_enabled(DeviceHandler *handle,
                                          uint8_t slave_id,
                                          uint8_t module_id,
                                          bool enabled);
...
int stark_revo3_get_touch_module_enabled(DeviceHandler *handle,
                                         uint8_t slave_id,
                                         uint8_t module_id);
...
void stark_revo3_set_motor_position(DeviceHandler *handle,
                                    uint8_t slave_id,
                                    uint16_t motor_id,
                                    float position_deg);

/// Set target current for a single Revo3 motor (Amperes).
void stark_revo3_set_motor_current(DeviceHandler *handle,
                                   uint8_t slave_id,
                                   uint16_t motor_id,
                                   float current);
...
void stark_revo3_set_motor_mit(DeviceHandler *handle,
                               uint8_t slave_id,
                               uint16_t motor_id,
                               float position_deg,
                               float velocity,
...
void stark_revo3_set_all_motor_positions(DeviceHandler *handle,
                                         uint8_t slave_id,
                                         const float *positions);

/// Set target velocity for a single Revo3 motor (RPM).
void stark_revo3_set_motor_velocity(DeviceHandler *handle,
                                    uint8_t slave_id,
                                    uint16_t motor_id,
                                    float velocity);
...
void stark_revo3_set_all_motor_velocities(DeviceHandler *handle,
                                          uint8_t slave_id,
                                          const float *velocities);

/// Set all 21 Revo3 motor currents at once (Amperes).
/// `currents` must point to an array of at least 21 f32 values.
void stark_revo3_set_all_motor_currents(DeviceHandler *handle,
                                        uint8_t slave_id,
                                        const float *currents);

/// Clear all Revo3 motor error states.
void stark_revo3_clear_motor_errors(DeviceHandler *handle, uint8_t slave_id);

/// Set Revo3 calibration current.
void stark_revo3_set_calibration_current(DeviceHandler *handle, uint8_t slave_id, float current);

/// Set Revo3 max continuous current.
void stark_revo3_set_max_continuous_current(DeviceHandler *handle, uint8_t slave_id, float current);

/// Read complete Revo3 motor status data.
/// Returns NULL on failure. Call `free_revo3_motor_status_data` to release.
CRevo3MotorStatusData *stark_revo3_get_motor_status_data(DeviceHandler *handle, uint8_t slave_id);

/// Read all 21 Revo3 motor positions (degrees).
/// `out_positions` must point to an array of at least 21 f32 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_all_motor_positions(DeviceHandler *handle,
                                        uint8_t slave_id,
                                        float *out_positions);
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_all_motor_velocities(DeviceHandler *handle,
                                         uint8_t slave_id,
                                         float *out_velocities);

/// Read all 21 Revo3 motor currents.
/// `out_currents` must point to an array of at least 21 f32 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_all_motor_currents(DeviceHandler *handle,
                                       uint8_t slave_id,
                                       float *out_currents);
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_all_motor_errors(DeviceHandler *handle, uint8_t slave_id, uint16_t *out_errors);

/// Free Revo3 motor status data returned by `stark_revo3_get_motor_status_data`.
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_all_motor_status(DeviceHandler *handle,
                                     uint8_t slave_id,
                                     uint16_t *out_statuses);

/// Set all Revo3 touch modules enabled/disabled via bitmask.
/// Bits 0~10 correspond to modules 0~10.
void stark_revo3_set_all_touch_modules_enabled(DeviceHandler *handle,
                                               uint8_t slave_id,
                                               uint16_t enabled_bits);

/// Set Revo3 touch data output type.
/// data_type: 0=AD raw, 1=Calibrated
void stark_revo3_set_touch_data_type(DeviceHandler *handle, uint8_t slave_id, uint16_t data_type);

/// Read Revo3 touch data output type.
/// Returns 0=AD raw, 1=Calibrated, or -1 on failure.
int stark_revo3_get_touch_data_type(DeviceHandler *handle, uint8_t slave_id);

/// Read Revo3 touch summary (16 pad force values).
/// `out_summary` must point to an array of at least 16 u16 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_touch_summary(DeviceHandler *handle, uint8_t slave_id, uint16_t *out_summary);

/// Read Revo3 touch module pressure data.
/// `out_data` must point to a buffer large enough (max 46 u16 for palm, 16 for fingers).
/// `out_count` will be set to the number of values written.
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_touch_module_data(DeviceHandler *handle,
                                      uint8_t slave_id,
                                      uint8_t module_id,
                                      uint16_t *out_data,
                                      uint16_t *out_count);
...
void stark_revo3_single_joint_control(DeviceHandler *handle,
                                      uint8_t slave_id,
                                      uint16_t joint_id,
                                      uint16_t mode,
                                      float param);
...
void stark_revo3_multi_joint_control(DeviceHandler *handle,
                                     uint8_t slave_id,
                                     uint16_t mode,
                                     const float *params);

/// Batch MIT: set all 21 Kp values (registers 1300–1320).
/// `kp_values` must point to at least 21 f32 values.
void stark_revo3_set_all_mit_kp(DeviceHandler *handle, uint8_t slave_id, const float *kp_values);

/// Batch MIT: set all 21 Kd values.
/// `kd_values` must point to at least 21 f32 values.
void stark_revo3_set_all_mit_kd(DeviceHandler *handle, uint8_t slave_id, const float *kd_values);

/// Batch MIT: set all 21 position values.
/// `positions` must point to at least 21 f32 values.
void stark_revo3_set_all_mit_positions(DeviceHandler *handle,
                                       uint8_t slave_id,
                                       const float *positions);

/// Batch MIT: set all 21 velocity values.
/// `velocities` must point to at least 21 f32 values.
void stark_revo3_set_all_mit_velocities(DeviceHandler *handle,
                                        uint8_t slave_id,
                                        const float *velocities);

/// Batch MIT: set all 21 torque values (mA).
/// `torques` must point to at least 21 f32 values.
void stark_revo3_set_all_mit_torques(DeviceHandler *handle, uint8_t slave_id, const float *torques);

/// Move a single joint using quintic polynomial trajectory.
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_joint(DeviceHandler *handle,
                           uint8_t slave_id,
                           uint16_t joint_id,
                           float target_pos,
                           float duration,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_joint_with_gains(DeviceHandler *handle,
                                      uint8_t slave_id,
                                      uint16_t joint_id,
                                      float target_pos,
                                      float duration,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_joint_with_speed(DeviceHandler *handle,
                                      uint8_t slave_id,
                                      uint16_t joint_id,
                                      float target_pos,
                                      float speed,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_joint_with_speed_and_gains(DeviceHandler *handle,
                                                uint8_t slave_id,
                                                uint16_t joint_id,
                                                float target_pos,
                                                float speed,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_hand(DeviceHandler *handle,
                          uint8_t slave_id,
...
/// `target_positions` must point to an array of `num_joints` f32 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_hand_with_gains(DeviceHandler *handle,
                                     uint8_t slave_id,
                                     const float *target_positions,
                                     uint16_t num_joints,
                                     float duration,
...
/// `target_positions` must point to an array of `num_joints` f32 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_hand_with_speed(DeviceHandler *handle,
                                     uint8_t slave_id,
                                     const float *target_positions,
                                     uint16_t num_joints,
                                     float speed,
...
/// `target_positions` must point to an array of `num_joints` f32 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_move_hand_with_speed_and_gains(DeviceHandler *handle,
                                               uint8_t slave_id,
                                               const float *target_positions,
                                               uint16_t num_joints,
                                               float speed,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_all_motor_temperatures(DeviceHandler *handle,
                                           uint8_t slave_id,
                                           uint16_t *out_temps);

/// Set global protection current (mA).
void stark_revo3_set_global_protect_current(DeviceHandler *handle,
                                            uint8_t slave_id,
                                            float current_ma);

/// Enable or disable software e-stop.
void stark_revo3_set_software_e_stop(DeviceHandler *handle, uint8_t slave_id, bool enabled);

/// Get broadcast ID usage state. Returns 1 for true, 0 for false, -1 on error.
int stark_revo3_get_use_broadcast_id(DeviceHandler *handle, uint8_t slave_id);

/// Get auto calibration state. Returns 1 for true, 0 for false, -1 on error.
int stark_revo3_get_auto_calibration(DeviceHandler *handle, uint8_t slave_id);

/// Get global protection current (mA). Returns 0 on success, -1 on error.
int stark_revo3_get_global_protect_current(DeviceHandler *handle,
                                           uint8_t slave_id,
                                           float *out_current);

/// Get max continuous current (mA). Returns 0 on success, -1 on error.
int stark_revo3_get_max_continuous_current(DeviceHandler *handle,
                                           uint8_t slave_id,
                                           float *out_current);

/// Get auto clear motor errors state. Returns 1 for true, 0 for false, -1 on error.
int stark_revo3_get_auto_clear_motor_error(DeviceHandler *handle, uint8_t slave_id);

/// Get teaching mode state. Returns 1 for true, 0 for false, -1 on error.
int stark_revo3_get_teaching_mode(DeviceHandler *handle, uint8_t slave_id);

/// Get touch screen switch state. Returns 1 for true, 0 for false, -1 on error.
int stark_revo3_get_touch_screen(DeviceHandler *handle, uint8_t slave_id);

/// Get software e-stop state. Returns 1 for true, 0 for false, -1 on error.
int stark_revo3_get_software_e_stop(DeviceHandler *handle, uint8_t slave_id);

/// Set per-joint protection current (mA).
void stark_revo3_set_joint_protect_current(DeviceHandler *handle,
                                           uint8_t slave_id,
                                           uint16_t joint_id,
                                           float current_ma);
...
/// Returns NULL on failure.
const char *stark_revo3_get_hardware_version(DeviceHandler *handle, uint8_t slave_id);

/// Get motor online status (bitmask).
/// Returns bitmask as u32, or 0 on failure.
uint32_t stark_revo3_get_motor_online_status(DeviceHandler *handle, uint8_t slave_id);

/// Decode a Revo3 motor status bitmask into a human-readable C string.
/// Returns a C string pointer; call `free_string` to release.
const char *stark_revo3_motor_status_desc(uint16_t status);
...
CRevo3SystemStatus *stark_revo3_get_system_status(DeviceHandler *handle, uint8_t slave_id);

/// Read system current (mA). Returns 0xFFFF on failure.
uint16_t stark_revo3_get_system_current(DeviceHandler *handle, uint8_t slave_id);

/// Read system voltage (V). Returns 0xFFFF on failure.
uint16_t stark_revo3_get_system_voltage(DeviceHandler *handle, uint8_t slave_id);

/// Read system power (W). Returns 0xFFFF on failure.
uint16_t stark_revo3_get_system_power(DeviceHandler *handle, uint8_t slave_id);

/// Read system temperature (°C). Returns 0xFFFF on failure.
uint16_t stark_revo3_get_system_temperature(DeviceHandler *handle, uint8_t slave_id);

/// Free system status data returned by `stark_revo3_get_system_status`.
...
uint16_t stark_revo3_get_all_touch_modules_enabled(DeviceHandler *handle, uint8_t slave_id);

/// Clear pressure data for all Revo3 touch modules.
void stark_revo3_reset_all_touch_pressure(DeviceHandler *handle, uint8_t slave_id);

/// Clear pressure data for a single Revo3 touch module.
void stark_revo3_reset_touch_pressure(DeviceHandler *handle, uint8_t slave_id, uint8_t module_id);

/// MIT control for a single joint.
/// τ = Kp*(pos_ref − pos) + Kd*(vel_ref − vel) + τ_ff
void stark_revo3_mit_control(DeviceHandler *handle,
                             uint8_t slave_id,
                             uint16_t joint_id,
                             float kp,
                             float kd,
...
/// Multi-MIT: set parameters for a single joint (registers 1100–1204).
void stark_revo3_multi_mit_set_joint(DeviceHandler *handle,
                                     uint8_t slave_id,
                                     uint16_t joint_id,
                                     float kp,
                                     float kd,
...
/// Multi-MIT: set all 21 joints at once.
/// All arrays must point to at least 21 f32 values.
void stark_revo3_multi_mit_set_all(DeviceHandler *handle,
                                   uint8_t slave_id,
...
/// Same as stark_revo3_multi_mit_set_all but without retry (for high-freq control).
void stark_revo3_multi_mit_set_all_without_retry(DeviceHandler *handle,
                                                 uint8_t slave_id,
                                                 const float *kp_values,
                                                 const float *kd_values,
                                                 const float *positions,
...
/// Set all 5 MIT batch parameters in a single write (105 registers).
void stark_revo3_set_all_mit_batch(DeviceHandler *handle,
                                   uint8_t slave_id,
                                   const float *kp_values,
                                   const float *kd_values,
                                   const float *positions,
...
/// Same as stark_revo3_set_all_mit_batch but without retry.
void stark_revo3_set_all_mit_batch_without_retry(DeviceHandler *handle,
                                                 uint8_t slave_id,
                                                 const float *kp_values,
                                                 const float *kd_values,
                                                 const float *positions,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_teach_joint(DeviceHandler *handle,
                            uint8_t slave_id,
                            uint16_t joint_id,
                            float dt,
                            float duration,
...
/// `out_count` will be set to the number of samples actually recorded.
/// Returns 0 on success, -1 on failure.
int stark_revo3_teach_hand(DeviceHandler *handle,
                           uint8_t slave_id,
                           float dt,
                           float duration,
                           uint16_t num_joints,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_replay_joint(DeviceHandler *handle,
                             uint8_t slave_id,
                             uint16_t joint_id,
                             const float *positions,
                             uint32_t count,
...
/// Returns 0 on success, -1 on failure.
int stark_revo3_replay_hand(DeviceHandler *handle,
                            uint8_t slave_id,
                            const float *positions,
                            uint32_t num_frames,
                            uint16_t num_joints,
...
int stark_revo3_admittance_joint(DeviceHandler *handle,
                                 uint8_t slave_id,
                                 uint16_t joint_id,
...
/// Returns 0 on success, -1 on failure (currently always -1).
int stark_revo3_admittance_hand(DeviceHandler *handle,
                                uint8_t slave_id,
...
void stark_revo3_finger_control(DeviceHandler *handle,
                                uint8_t slave_id,
                                uint8_t finger_id,
                                uint16_t mode,
                                const float *params);
...
void stark_revo3_thumb_control(DeviceHandler *handle,
                               uint8_t slave_id,
                               uint16_t mode,
                               const float *params);
...
uint16_t stark_revo3_get_motor_temperature(DeviceHandler *handle,
                                           uint8_t slave_id,
                                           uint16_t motor_id);
...
/// Returns a C string pointer; call `free_string` to release.
/// Returns NULL on failure.
const char *stark_revo3_get_motor_sn(DeviceHandler *handle, uint8_t slave_id, uint16_t motor_id);

/// Get all motor firmware versions (21 values).
/// `out_versions` must point to an array of at least 21 u16 values.
/// Returns 0 on success, -1 on failure.
int stark_revo3_get_motor_fw_versions(DeviceHandler *handle,
                                      uint8_t slave_id,
